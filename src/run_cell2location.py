...
def read_sc(count,meta,celltype_key='cell_type',batch_key=None,categorical_covariate_key=None,continuous_covariate_key=None):
    ...
    if categorical_covariate_key != None and not all(x in count.obs.columns for x in categorical_covariate_key):
        raise KeyError('categorical_covariate_key ' + categorical_covariate_key + 'is not a subset of ' + ', '.join(count.obs.columns) + '!')
    if continuous_covariate_key != None and not all(x in count.obs.columns for x in continuous_covariate_key):
        raise KeyError('continuous_covariate_key ' + continuous_covariate_key + 'is not a subset of ' + ', '.join(count.obs.columns) + '!')

    return count

class PerformCell2loc:
    def __init__(self,adata_sp,adata_sc,output_dir,celltype_key='cell_type',batch_key='batch',categorical_covariate_key=None,continuous_covariate_key=None,N_cells_per_location=30,detection_alpha=20):
        #### Filter
...
def main(argsv):
    ...
    parser.add_argument("--celltype_key", type=str,default="cell_type", help='The column name indicating cell type in scRNA-seq metadata table')
    parser.add_argument("--batch_key", type=str,default=None, help='The column name indicating batch effect in scRNA-seq metadata table')
    parser.add_argument("--categorical_covariate_key", type=str,nargs="+", default=None, help='The column name indicating categorical covariate, this argument can be passed in a continuous way')
    parser.add_argument("--continuous_covariate_key", type=str,nargs="+", default=None, help='The column name indicating continuous covariate, this argument can be passed in a continuous way')
    ...
    if args.label != "":
        input = os.path.join(args.input_path, args.dataset, args.section, '.'.join([args.section,args.label,"process_st.h5ad"]))
    else:
